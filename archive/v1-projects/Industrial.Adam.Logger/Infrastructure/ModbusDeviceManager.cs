...
internal class ModbusDeviceManager : IModbusDeviceManager
{
    ...
    public async Task<bool> ConnectAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Complete connection and Modbus setup with retry logic for Linux socket reuse issues
            ...
            for (int attempt = 0; attempt < 3; attempt++)
            {
                ...
                try
                {
                    // Always create a fresh TCP client to avoid socket reuse issues on Linux
                    ...
                    // Create and configure Modbus master immediately after connection
                    var factory = new ModbusFactory();
                    modbusMaster = factory.CreateMaster(tcpClient);
                    modbusMaster.Transport.ReadTimeout = _config.TimeoutMs;
                    modbusMaster.Transport.WriteTimeout = _config.TimeoutMs;
                    modbusMaster.Transport.Retries = Math.Max(0, _config.MaxRetries);
                    
                    // Success - store both objects and break out of retry loop
                    _tcpClient = tcpClient;
                    _modbusMaster = modbusMaster;
                    tcpClient = null; // Prevent disposal
                    modbusMaster = null; // Prevent disposal
                    break;
                }
                catch (Exception ex) when (ex is SocketException || ex is PlatformNotSupportedException || ex is TimeoutException || ex is InvalidOperationException)
                {
                    lastException = ex;
                    _logger.LogDebug("Connection attempt {Attempt} failed for {DeviceId}: {Error}", 
                        attempt + 1, _config.DeviceId, ex.Message);
                    
                    // Clean up failed connection and Modbus master
            ...
            {
                throw lastException ?? new SocketException((int)SocketError.ConnectionRefused);
            }
