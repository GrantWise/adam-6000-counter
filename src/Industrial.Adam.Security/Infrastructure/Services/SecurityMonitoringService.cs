...
public class SecurityMonitoringService : BackgroundService
{
    ...
    private async Task GenerateAlerts(CancellationToken cancellationToken)
    {
            ...
            metrics.AuthenticationSuccessRate < _options.MinSuccessRate)
        {
            ...
            var alert = new SecurityAlert
            {
                ...
                Severity = ConvertToAlertSeverity(SecurityEventSeverity.Medium),
        ...
        if (metrics.HighRiskEvents > _options.HighRiskEventThreshold)
        {
            ...
            var alert = new SecurityAlert
            {
                ...
                Severity = ConvertToAlertSeverity(SecurityEventSeverity.High),
    ...
    /// <summary>
    /// Converts SecurityEventSeverity to SecurityAlertSeverity
    /// </summary>
    /// <param name="eventSeverity">Event severity to convert</param>
    /// <returns>Corresponding alert severity</returns>
    private static SecurityAlertSeverity ConvertToAlertSeverity(SecurityEventSeverity eventSeverity)
    {
        return eventSeverity switch
        {
            SecurityEventSeverity.Information => SecurityAlertSeverity.Low,
            SecurityEventSeverity.Low => SecurityAlertSeverity.Low,
            SecurityEventSeverity.Warning => SecurityAlertSeverity.Medium,
            SecurityEventSeverity.Medium => SecurityAlertSeverity.Medium,
            SecurityEventSeverity.High => SecurityAlertSeverity.High,
            SecurityEventSeverity.Critical => SecurityAlertSeverity.Critical,
            _ => SecurityAlertSeverity.Low
        };
    }
