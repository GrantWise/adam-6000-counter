...
### Step 2: Start TimescaleDB with Correct Credentials
```bash
...
### Step 3: Start Backend Services with Environment Variables
```bash
# Terminal 1: Logger API (Port 5000)
...
**For comprehensive troubleshooting, see [Setup Troubleshooting Guide](docs/SETUP_TROUBLESHOOTING.md)**

### ❌ Critical Setup Issues

**"Environment variables not being substituted"**
- Problem: Configuration contains literal `${VARIABLE}` instead of values
- Solution: Use inline environment variables as shown in Step 3 above

**"Security validation failures"**
- Problem: Default credentials rejected by security system
- Solution: Use complex credentials: `industrial_system` / `IndustrialCounter2024!@#$`

**"Port conflicts"**
- Problem: Multiple services trying to use same port
- Solution: Use specific ports - Logger: 5000, OEE: 5001
...
psql postgresql://industrial_system:IndustrialCounter2024!@#$@localhost:5433/adam_counters -c "SELECT 1;"  # Database
```
